...
def dmg_col_10_2D(dmglvl,colnum,lock,dmgpct,d1,d2,d3,d4,pwr,IPR,freq,eigfreq,d1sd,d2sd,d3sd,\
...
#    with lock:
    dmgpct[Ndamage*colnum+dmglvl]=np.sum(damage)/H0_tot
    IPR[Ndamage*colnum+dmglvl] = np.average(IPRit)      #Had to do 1D list, so 
    pwr[Ndamage*colnum+dmglvl] = np.average(pwrit)              #it goes column 0 damage lvl
    freq[Ndamage*colnum+dmglvl]=np.average(frequencyit)         #0,1,2,3,4...Ndamage-1, then
                                                        #col 1 damage level 0,1,2...
    IPRsd[Ndamage*colnum+dmglvl]=np.std(IPRit)
    pwrsd[Ndamage*colnum+dmglvl]=np.std(pwrit)
    freqsd[Ndamage*colnum+dmglvl]=np.std(frequencyit)
    
    d1[Ndamage*colnum+dmglvl],d1sd[Ndamage*colnum+dmglvl] = np.average(d1it),np.std(d1it)
    d2[Ndamage*colnum+dmglvl],d2sd[Ndamage*colnum+dmglvl] = np.average(d2it),np.std(d2it)
    d3[Ndamage*colnum+dmglvl],d3sd[Ndamage*colnum+dmglvl] = np.average(d3it),np.std(d3it)
    d4[Ndamage*colnum+dmglvl],d4sd[Ndamage*colnum+dmglvl] = np.average(d4it),np.std(d4it)
    

    eigfreq[Ndamage*colnum+dmglvl] = np.copy(freq2)
...
if __name__ == '__main__':
    ...
    processes = []
    lock = Lock()
    for lvl in np.arange(Ndamage):
        for coln in np.arange(Nmitral):
            proc = Process(target = dmg_col_10_2D, args=(lvl,coln,lock,dmgpct,d1,\
