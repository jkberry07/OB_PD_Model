...
#    spread = -1 #start at -1 so that the first damage level has a spread of 0 radius
    ...
    for lv in np.arange(Ndamage):
        ...
        if lv>0: 
            if np.mod(lv,2) == 1 and lv>2:
                newcols = []
                for i in dmgcols:
                    newcols.extend([int(np.mod(i+1,4)),int(np.mod(i-1,4)),\
                           int(np.mod(i+4,20)),int(np.mod(i-4,20))])
                for n in newcols:
                    if not(n in dmgcols):
                        dmgcols.extend([n])
...
#        d4sd[lv]=np.std(d4it)
    ...
    return dmgpct1,eigfreq1,d11,d21,d31,d41,pwr1,IPR1,IPR2,pn1,freq1
...
#    Ncolumns = np.copy(Nmitral)
    ...
    output = pool.map(dmg_seed_20_2D,columns)
    pool.close()
    pool.join()
    dmgpct = [output[i][0][:] for i in np.arange(Ncolumns)]
    eigfreq = [output[i][1][:] for i in np.arange(Ncolumns)]
    d1 = [output[i][2][:] for i in np.arange(Ncolumns)]
    d2 = [output[i][3][:] for i in np.arange(Ncolumns)]
    d3 = [output[i][4][:] for i in np.arange(Ncolumns)]
    d4 = [output[i][5][:] for i in np.arange(Ncolumns)]
    pwr = [output[i][6][:] for i in np.arange(Ncolumns)]
    IPR = [output[i][7][:] for i in np.arange(Ncolumns)]
    IPR2 = [output[i][8][:] for i in np.arange(Ncolumns)]
    pn = [output[i][9][:] for i in np.arange(Ncolumns)]
    freq = [output[i][10][:] for i in np.arange(Ncolumns)]
