...
#    P_odor3 = 4/700*np.array([.7,.8,.5,1.2,.7,1.2,.8,.7,.8,.8])
    ...
    H0 = np.load('H0_10_2D_60Hz.npy')   #load weight matrix
    
    H0_tot = np.sum(H0)         #get sum of the weights
    
    W0 = np.load('W0_10_2D_60Hz.npy')  #load weight matrix
...
#    Ncolumns = np.copy(Nmitral)
    ...
    dmgpct,eigfreq,d1,d2,d3,d4,pwr,IPR,IPR2,pn,freq = dmg_col_10_2D(arrayid)
