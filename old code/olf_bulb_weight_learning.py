...
from olf_diffeq import lrndiffeq_all
...
#    Ag[i] = np.random.normal(.8,.1)     #for transforming target
    Ag[i]= np.random.normal(.05,.01)
    while Ag[i]<0:
        Ag[i]= np.random.normal(.2,.075)
#    Cg[i] = np.random.normal(.0025,.00033)
    Bg[i] = np.random.normal(.2,.1)
    while Bg[i]<0:
        Bg[i] = np.random.normal(.2,.08)
...
H0[H0>0]=np.random.rand(np.size(H0[H0>0]))
W0[W0>0]=np.random.rand(np.size(W0[W0>0]))
...
sol = solve_ivp(lambda t,y: lrndiffeq_Codor(t,y,target,Nmitral,Ngranule,Ndim,\
            t_inh,t_exh,exh_rate,alpha,Sy,Sy2,Sx,Sx2,th,H0,W0,locsH0,locsW0,P_odor1,Ic,Ib,\
            teval,Am,Bm,phaseshift,Ag,Cg,Bg,eta),\
            [0,final_lt],init0,t_eval = teval,method = 'RK45') 
