...
H0s = np.zeros((Nweights,np.shape(H0)[0],np.shape(H0)[1]))
W0s = np.zeros((Nweights,np.shape(W0)[0],np.shape(W0)[1]))

for p in np.arange(Nweights):
    H0fl = "H0_" + str(p) + ".npy"
    W0fl = "W0_" + str(p) + ".npy"
    H0s[p,:,:] = np.load(H0fl)
    W0s[p,:,:] = np.load(W0fl)
...
Ndiffs = np.math.factorial(Nweights)/(np.math.factorial(2)*np.math.factorial(Nweights-2))
