...
#    Ndamagetotal = Nmitral*2 + 1  #number of damage steps
    Ndamage = 21 #steps to reduce entire matrix to zero
...
#    P_odor3 = 4/700*np.array([.7,.8,.5,1.2,.7,1.2,.8,.7,.8,.8])
    ...
    d11,d11sd = np.zeros(Ndamage),np.zeros(Ndamage)
    d21 = np.zeros(Ndamage)
    d31,d31sd = np.zeros(Ndamage),np.zeros(Ndamage)
    d41 = np.zeros(Ndamage)
    pwr1,pwr1sd = np.zeros(Ndamage),np.zeros(Ndamage)
    ...
    for lv in np.arange(Ndamage):
        ...
        if lv>0: 
            Hdamaged = Hdamaged-Hdamagestep  
            Hdamaged[Hdamaged<0] = 0
            damage = np.sum(H0-Hdamaged)
...
#        print('level',lv)
        ...
        pwr1sd[lv] = np.std(pwrit)
        freq1[lv]=np.average(frequencyit)         
...
#        freqsd[lv]=np.std(frequencyit)
        ...
        d11sd[lv] =  np.std(d1it)
#        d2sd[lv] = np.std(d2it)
        d31sd[lv]=np.std(d3it)
#        d4sd[lv]=np.std(d4it)
        
        eigfreq1[lv] = np.copy(freq2)
        cell_act[:,:,lv]=np.copy(yout2)
   
    return dmgpct1,eigfreq1,d11,d21,d31,d41,pwr1,IPR1,IPR2,pn1,freq1,cell_act,d11sd,d31sd,pwr1sd
...
#    Ncolumns = np.copy(Nmitral)
    ...
    dmgpct,eigfreq,d1,d2,d3,d4,pwr,IPR,IPR2,pn,freq,cell_act,d1sd,d3sd,pwrsd = dmg_seed_50_1D(arrayid)  
...
#    "d2sd","d3sd","d4sd","IPRsd","pwrsd","frequencysd"
    ...
    np.save("d1sd"+pd_type,d1sd),np.save("d3sd"+pd_type,d3sd),np.save("pwrsd"+pd_type,pwrsd)
