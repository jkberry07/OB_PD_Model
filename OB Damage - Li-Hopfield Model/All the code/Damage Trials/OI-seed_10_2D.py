...
def equi(x,Ndim,Nmitral,Sx,Sx2,Sy,Sy2,th,alpha,t_inh,H0,W0,P_odor,Ib,Ic,dam):
    ...
    F[0:Nmitral] = (np.ravel(-np.dot(H0,gy)) + np.ravel(Ib) + \
        np.ravel(dam*P_odor*(180-t_inh))) - np.ravel(alpha*x[0:Nmitral]) #180 is 25 ms before the end of inhale
    F[Nmitral:] = (np.ravel(np.dot(W0,gx)) + np.ravel(Ic)) - np.ravel(alpha*x[Nmitral:])
    return F
...
def diffeq(t,x,Nmitral,Ngranule,Ndim,lastnoise,noise,noisewidth,noiselevel,\
    ...
    for i in np.r_[0:Nmitral]:
        if t < t_inh:
            ...
            dydt[i] = ((t-lastnoise[i])*noise[i] - np.dot(np.reshape(H0[i,:],\
                (1,Ngranule)), cellout(y[Nmitral:],Sy,Sy2,th)) + \
                Ib[i] + dam[i]*P_odor[i]*(t-t_inh)) - alpha*y[i]
        else:
            dydt[i] = ((t-lastnoise[i])*noise[i] - np.dot(np.reshape(H0[i,:],\
                (1,Ngranule)), cellout(y[Nmitral:],Sy,Sy2,th)) + \
                Ib[i] + dam[i]*P_odor[i]*(t-t_inh)*np.exp(-exh_rate*(t-t_exh))) - alpha*y[i]
...
#    P_odor3 = 4/700*np.array([.7,.8,.5,1.2,.7,1.2,.8,.7,.8,.8])
    ...
    H0 = np.load('H0_10_2D_65Hz.npy')   #load weight matrix
    
    W0 = np.load('W0_10_2D_65Hz.npy')  #load weight matrix
    ...
    for lv in np.arange(Ndamage):
        ...
        if lv>0: 
            if lv>1:
                newcols = []
                for i in dmgcols:
                    newcols.extend([int(np.mod(i+1,10)),int(np.mod(i-1,10)),\
                           int(np.mod(i+4,Nmitral)),int(np.mod(i+5,Nmitral))])
                for n in newcols:
                    if not(n in dmgcols):
                        dmgcols.extend([n])
            dam[dmgcols] = dam[dmgcols] - 0.1   
            dam[dam<1e-10] = 0
            damage = np.sum(1-dam)
...
#        print('level',lv)
        ...
        dmgpct1[lv]=damage/Nmitral
        IPR1[lv] = np.average(IPRit)      #Had to do 2D list, so 
...
#    Ncolumns = np.copy(Nmitral)
    ...
    dmgpct,eigfreq,d1,d2,d3,d4,pwr,IPR,IPR2,pn,freq,cell_act = dmg_seed_10_2D(arrayid)  
