...
#    Sbp_b,Sbp_gam = np.zeros((finalt,Ndim)),np.zeros((finalt,Ndim))
    ...
    damage = 0
    dam[colnum]+=.5 #so that it starts with zero damage
    for col in range(Ncols):            
        cols = int(np.mod(colnum+col,Nmitral)) #assign the column to be damaged
        ...
        #        pwr_b[counter] = np.average(pwr_bit)
        #        pwr_gam[counter] = np.average(pwr_gamit)
