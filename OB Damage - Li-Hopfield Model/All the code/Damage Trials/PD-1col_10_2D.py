    ...
    for col in range(Ncols):            
        ...
        for lv in np.arange(Ndamage):
            ...
            if not(lv==0 and cols!=colnum):     #if it's the 0th level for any but the original col, skip
                Hdamaged[:,cols] = H0[:,cols]*(1-lv*(0.1))
        ...
        #        print('level',lv)
                ...
                dmgpct1[counter]=np.sum(H0 - Hdamaged)/H0_tot
...
#    Ncolumns = np.copy(Nmitral)
    ...
    dmgpct,eigfreq,d1,d2,d3,d4,pwr,IPR,IPR2,pn,freq,cell_act = dmg_col_10_2D(arrayid)
