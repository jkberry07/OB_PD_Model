...
#    Ndamagetotal = Nmitral*2 + 1  #number of damage steps
    Ndamage = 36 #steps to reduce entire matrix to zero
...
#    P_odor3 = 4/700*np.array([.7,.8,.5,1.2,.7,1.2,.8,.7,.8,.8])
    ...
    for lv in np.arange(Ndamage):
        ...
        for m in np.arange(M):
            ...
            d1it[m] = d1it[m]/M  #average over comparison with all control trials
            d2it[m] = d2it[m]/M
            d3it[m] = d3it[m]/M
            d4it[m] = d4it[m]/M
...
#    Ncolumns = np.copy(Nmitral)
    ...
    dmgpct,eigfreq,d1,d2,d3,d4,pwr,IPR,IPR2,pn,freq,cell_act = dmg_seed_50_1D(arrayid)  
